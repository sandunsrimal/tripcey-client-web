...
import styles from './styles/AttractionForm.module.css';
...
const categories = ["Historical", "Natural", "Religious", "Adventure", "Entertainment"]; // Add all categories
...
const AttractionForm = ({ onNext, setHotelId, hotelId }) => {
  ...
  }, [primaryImage, secondaryImages, address, clearErrors]);

  useEffect(() => {
    const fetchHotelData = async () => {
      if (hotelId) {
        try {
          const docRef = doc(db, 'tripcey-attractions', hotelId);
  ...
  const generateHotelId = () => `attraction_${Date.now()}_${Math.floor(Math.random() * 10000)}`;
  ...
  const uploadImageToFirebase = async (image, hotelId) => {
    ...
    const imageRef = ref(storage, `attraction-images/${hotelId}/${imageName}`);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!hotelId) {
        ...
        await setDoc(doc(db, "tripcey-attractions", hId), {
          ...data,
          address,
          id: hId,
          lat: location.lat,
        ...
        await updateDoc(doc(db, "tripcey-attractions", hId), {
          ...data,
          address,
          lat: location.lat,
          lon: location.lon,
        ...
        <div className={styles.hotelformGroup}>
          <div className={styles.leftSide}>
            <div className={styles.formGroup}>
              <label className={styles.label}>Attraction Name <span className={styles.required}>*</span></label>
              <input {...register('name', { required: true })} className={styles.input} placeholder="Enter attraction name" />
...
export default dynamic(() => Promise.resolve(AttractionForm), { ssr: false });
